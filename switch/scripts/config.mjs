...
const sharedReleaseVersion = readFileSync(
  new URL("../../configs/release-version.txt", import.meta.url),
  "utf8",
).trim();
...
// CI exports this same value, while local builds read it directly so every
// packaged platform has one user-edited release-version source.
export const SILVERSHADOW_VERSION = process.env.SILVERSHADOW_VERSION || sharedReleaseVersion;
